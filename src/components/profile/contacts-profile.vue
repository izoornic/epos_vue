...
export default {
  ...
  data() {
    return {
      title: this.$t("navbar.dropdown.henry.list.profile"),
      items: [
        {
        ...
        {
          text: this.$t("navbar.dropdown.henry.list.profile"),
          active: true,
        },
      ],
      statData: [
  ...
  validations: {
    form:
      store.state.authapi.user != null &&
      store.state.authapi.user.roleName.toLowerCase() === "merchant"
        ? {
  ...
  mounted() {
    ...
    if (
      this.$store.state.authapi.user != null &&
      this.$store.state.authapi.user.roleName.toLowerCase() === merchantRole
    ) {
      /**
       * For fetching records
       */
    ...
    if (
      this.$store.state.authapi.user != null &&
      this.$store.state.authapi.user.roleName.toLowerCase() === superadminRole
    ) {
      /**
       * For fetching records
       */
      this.fetchDataMerchantList();
    }
  },
  computed: {
    /**
  ...
  methods: {
    ...
    async formSubmit() {
      ...
      if (this.$v.$invalid) {
        ...
        if (
          ...
          //console.log("addMerchantData payload:" + JSON.stringify(payload));
          this.addMerchantData(payload);
          this.form.password="";
        } else {
          //console.log("updateUserData payload:" + JSON.stringify(payload));
          this.updateUserData(payload);
          this.form.password="";
        }
        EventBus.$emit("usernameval", this.form.merchant_Name);
    ...
    async fetchMerchantDetail() {
      ...
      await axios
        ...
        .then((response) => {
          ...
          // console.log("this.form", this.form);
          this.$store.dispatch("merchant/addMerchants", res);
          this.$store.state.authapi.user != null &&
          this.$store.state.authapi.user.merchant_Id != ""
            ? EventBus.$emit("usernameval", res.merchant_Name)
            : EventBus.$emit("usernameval", "Zeta System");
        })
        .catch((error) => {
          // this.isBusy = !this.isBusy;
    ...
    async fetchDataMerchantList() {
      this.isBusy = !this.isBusy;
      await axios
        .get(
          `${apiUrl}/merchant/getAllAdminMerchants?limit=${this.perPage}&page=${this.currentPage}&sortBy=${this.sortBy}&sortDirection=${this.sortDirection}`
        )
        .then((response) => {
          var res = JSON.parse(JSON.stringify(response.data));
          // console.log("res.items", res.items);
            ...
            <div class="table-responsive">
              <table class="table table-nowrap mb-0">
                <tbody>
                  ...
                  <tr v-if="this.$store.state.authapi.user != null">
                    <th scope="row">{{ $t("label.email") }} :</th>
                    <td>{{ this.$store.state.authapi.user.email }}</td>
                  </tr>
                  <tr
                    v-if="
                      this.$store.state.authapi.user != null &&
              ...
              >
                ...
                <template
                  ...
                  v-if="
                    ...
                    this.$store.state.authapi.user.roleName.toLowerCase() ===
                      'merchant'
                      ? posGridData && posGridData.length === 0
                      : merchantGridData && merchantGridData.length === 0
                  "
                >
                  <td
